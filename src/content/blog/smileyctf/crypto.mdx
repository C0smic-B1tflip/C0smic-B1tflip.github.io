...
```
This is a typical truncated LCG challenge, but with complex numbers. The main idea is to double the dimensions, as complex numbers have 2 by 2 matrix representations. We looked at a normal truncated LCG writeup [here](https://jsur.in/posts/2020-09-20-downunderctf-2020-writeups#lsb-msb-calculation-game) to build the main attack.
We replace \((x_1, x_2, \dots, x_{k+1})\) with \((\mathrm{Re}(x_1), \mathrm{Im}(x_1), \mathrm{Re}(x_2), \mathrm{Im}(x_2), \dots, \mathrm{Re}(x_{k+1}), \mathrm{Im}(x_{k+1})),\) with similar component doubling for the other vectors. The matrix in particular becomes \[
\begin{pmatrix}
             M &               0 &      0 &      0 &      0 &      0 &  \dots &      0 &      0 \\
             0 &               M &      0 &      0 &      0 &      0 &  \dots &      0 &      0 \\
\mathrm{Re}(a) & -\mathrm{Im}(a) &     -1 &      0 &      0 &      0 &  \dots &      0 &      0 \\
\mathrm{Im}(a) &  \mathrm{Re}(a) &      0 &     -1 &      0 &      0 &  \dots &      0 &      0 \\
\mathrm{Re}(a) & -\mathrm{Im}(a) &      0 &      0 &     -1 &      0 &  \dots &      0 &      0 \\
\mathrm{Im}(a) &  \mathrm{Re}(a) &      0 &      0 &      0 &     -1 &  \dots &      0 &      0 \\
        \vdots &          \vdots & \vdots & \vdots & \vdots & \vdots & \ddots & \vdots & \vdots \\
\mathrm{Re}(a) & -\mathrm{Im}(a) &      0 &      0 &      0 &      0 &  \dots &     -1 &      0 \\
\mathrm{Im}(a) &  \mathrm{Re}(a) &      0 &      0 &      0 &      0 &  \dots &      0 &     -1 \\
\end{pmatrix}.
\]
### Final Script
```py
...
M = 41385045268803457687081702183
a = 7677443135906757371086239158 + 1202973425456014711302222902*I
raw_hints = bytes.fromhex('')
ct = bytes.fromhex('3d2276bcefce8c3360b2f5fa87405d1d6da92b896fb746fa343439692a2d57e8')

# similar to y' in the link - every other dimension is a real component, going Re Im Re Im etc.
y_prime = []
for i in range(190):
    y_prime.append(raw_hints[i+2] - raw_hints[i+0])
y_prime = vector(y_prime)

# 190 by 190 matrix - one dimension for each x' (or y')
L = [[0 for _ in range(190)] for _ in range(190)]

# introduce multiple of M
for i in range(0, 2):
    L[i][i] = M

# -1 coefficient from a**i * x'[0] + (-1) * x'[i]
for i in range(2, 190):
    L[i][i] = -1

# complex multiplication by power of a (a**i coefficient)
for i in range(1, 95):
    L[2*i + 0][0] = (a**i)[0] % M
    L[2*i + 0][1] = (-(a**i)[1]) % M
    L[2*i + 1][0] = (a**i)[1] % M
...
# list -> Matrix -> reduced basis
L = Matrix(L)
B = L.LLL()

# number of bits truncated/lost
trunc = 88
# B * 2**trunc * y'
B_2_trunc_y_prime = B * 2**trunc * y_prime
# same as v in link - know B * z' = v * M - B * 2**trunc * y' is small
v = vector([round(i/M) for i in B_2_trunc_y_prime])
# B * z'
B_z_prime = (v * M) - B_2_trunc_y_prime
# cancel out B, find z'
z_prime = B.inverse() * B_z_prime

# reconstruct hint difference vector x'
x_prime = (2**trunc * y_prime) + z_prime
# reconstruct complex numbers
x_prime = [x_prime[i] + I*x_prime[i+1] for i in range(0, 190, 2)]

# extrapolate more hint differences as x' is geometric sequence
diffs = [a*(a**i)*x_prime[-1] for i in range(9)]
# approximate last LCG output using given bits
approx_last = (raw_hints[-2] << 88) + I*(raw_hints[-1] << 88)

# approximate key using extrapolated differences and approximated output
approx_key = []
for diff in diffs:
    approx_last += diff
    approx_last = (approx_last[0] % M) + I*(approx_last[1] % M)
    approx_key.append(int(approx_last[0]) >> trunc)
    approx_key.append(int(approx_last[1]) >> trunc)
approx_key = approx_key[2:] # just by local experimentation
#print(approx_key) # for debugging with local

# to account for possible error in approximating the key
for i in itertools.product(*[[0,1] for _ in range(16)]):
    # deriving key
    key = [thing + thing2 for thing, thing2 in zip(approx_key, i)]
    key = hashlib.sha256(bytes(key)).digest()
    # xor decryption
    msg = bytes([a ^^ b for a, b in zip(ct, key)])
    if all(chr(c) in printable for c in msg):
        print(msg)
```
